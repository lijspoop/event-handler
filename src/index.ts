import { EventDefinition, EventHandler, EventPrecondition, RegisterEventListenerType } from "./types";

export function defineHandler(handler: EventHandler) {
  handler.init?.();
  if ('events' in handler && Array.isArray(handler.events)) {
    handlerEvents(handler.events);
  }

  function handlerEvents(events: EventDefinition[] | undefined, parentEvent: EventDefinition | undefined = undefined) {
  ...
  function registerEventListener(type: RegisterEventListenerType, event: EventDefinition) {
    ...
    return function (evt?: Event) {
      if (
        ...
        (!Object.hasOwn(event, 'parentEvent') &&
          preventByCondition(handler?.preconditions))
      ) {
        return;
      }
